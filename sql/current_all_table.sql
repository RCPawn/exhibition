...
(
    category_id   bigint auto_increment comment '分类ID'
        primary key,
    parent_id     bigint      default 0                 null comment '父分类ID（0为顶级大类）',
    category_name varchar(100)                          not null comment '分类名称',
    category_code varchar(100)                          null comment '分类编码（可用于前端/接口过滤）',
    sort_order    int         default 0                 null comment '显示顺序（越小越靠前）',
    status        tinyint(1)  default 0                 null comment '状态（0=正常,1=停用）',
    del_flag      char        default '0'               null comment '删除标志（0=未删,2=已删）',
    ...
    comment '非遗分类表' collate = utf8mb4_unicode_ci;

create index idx_parent_id
    on heritage_category (parent_id);

create table heritage_comment
(
    comment_id  bigint auto_increment comment '评论ID'
        primary key,
    item_id     bigint           not null comment '非遗展品ID',
    user_id     bigint           not null comment '用户ID',
    parent_id   bigint           null comment '父评论ID(用于回复，预留)',
...
(
    inheritor_id bigint auto_increment comment '传承人ID'
        primary key,
    name         varchar(100)                          not null comment '姓名',
    avatar       varchar(500)                          null comment '个人照片',
    level        char                                  not null comment '级别（1国家级 2省级 3市级 4县级）',
    category_id  bigint                                not null comment '所属技艺分类ID（关联 heritage_category）',
    introduction text                                  null comment '个人简介',
    status       char        default '0'               null comment '状态（0在世 1去世）',
...
create index idx_category_id
    on heritage_inheritor (category_id);

create table heritage_item
(
    item_id        bigint auto_increment comment '展品ID'
        primary key,
    item_name      varchar(200)                           not null comment '展品名称',
    category_id    bigint                                 not null comment '分类ID（关联 heritage_category.category_id）',
    cover_image    varchar(500)                           null comment '封面图片URL（相对路径）',
    model_file     varchar(500)                           null comment '3D模型文件路径（.glb/.gltf 等）',
    description    text                                   null comment '展品简介（展示用）',
    history        text                                   null comment '历史渊源',
    craft          text                                   null comment '制作工艺',
    ...
    comment_count  bigint       default 0                 null comment '评论数量',
    status         tinyint(1)   default 0                 null comment '状态（0正常,1下架）',
    del_flag       char         default '0'               null comment '删除标志（0=未删,2=已删）',
...
create index idx_cat_status_sort
    on heritage_item (category_id, status, sort_order);

create index idx_category_id
    on heritage_item (category_id);

create index idx_favorite_count
    on heritage_item (favorite_count);
...
(
    action_id   bigint auto_increment comment '主键ID'
        primary key,
    user_id     bigint                             not null comment '用户ID',
    item_id     bigint                             not null comment '展品ID',
    action_type tinyint(1)                         not null comment '动作类型: 1-点赞, 2-收藏',
    create_time datetime default CURRENT_TIMESTAMP null comment '操作时间',
    constraint uk_user_item_type
